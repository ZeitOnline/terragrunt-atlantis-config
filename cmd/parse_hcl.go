...
func parseHclWithCache(filePath string) (*hcl.File, error) {
	...
	if parseErr != nil {
		// Cache the error too
		parsedHclCache.Store(filePath, parsedHclEntry{nil, parseErr, modTime})
		return nil, parseErr
	}

	// Handle bare include blocks similar to decodeHcl
	// Excluding json because of https://github.com/transcend-io/terragrunt-atlantis-config/issues/244.
	if filepath.Ext(filePath) != jsonFileExtension {
		updatedBytes, isUpdated, err := updateBareIncludeBlock(file, filePath)
		if err != nil {
			parsedHclCache.Store(filePath, parsedHclEntry{nil, err, modTime})
			return nil, err
		}
		if isUpdated {
			// Code was updated, so we need to reparse the new updated contents
			parser := getHCLParser()
			file, parseErr = parseHcl(parser, string(updatedBytes), filePath)
			putHCLParser(parser)
			if parseErr != nil {
				parsedHclCache.Store(filePath, parsedHclEntry{nil, parseErr, modTime})
				return nil, parseErr
